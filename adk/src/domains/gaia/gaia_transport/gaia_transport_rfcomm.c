...
typedef struct
{
    ...
    uint8 channel;          /*!< RFCOMM channel registered. */
    union {
        Sink sink;              /*!< Stream sink of this transport. */
        uint8 dlci;             /*!< DLCI sink (only used during marshaling->unmarshaling->commit). */
    } u;
...
static void gaiaTransport_RfcommDisconnectReq(gaia_transport *t)
{
    ...
    if (TransportMgrDisconnect(transport_mgr_type_rfcomm, tr->u.sink) != transport_mgr_status_success)
        Gaia_TransportDisconnectCfm(&tr->common, FALSE);
}
...
static void gaiaTransport_RfcommError(gaia_transport *t)
{
    gaia_transport_rfcomm_t *tr = (gaia_transport_rfcomm_t *)t;
    PanicNull(tr);
    DEBUG_LOG_ERROR("gaiaTransportRfcommError, sink %04x", tr->u.sink);
...
static bool gaiaTransport_RfcommHandoverMarshal(gaia_transport *t, uint8 *buf, uint16 buf_length, uint16 *written)
{
    ...
    if (buf_length >= sizeof(gaia_transport_rfcomm_marshalled_t))
    {
        ...
        md->dlci             = SinkGetRfcommDlci(tr->u.sink);  /* Get DLCI from sink as allocated server channel number might not be unique */
...
static bool gaiaTransport_RfcommHandoverUnmarshal(gaia_transport *t, const uint8 *buf, uint16 buf_length, uint16 *consumed)
{
    ...
    if (buf_length >= sizeof(gaia_transport_rfcomm_marshalled_t))
    {
        ...
        if (md.channel == tr->channel)
        {
            ...
            tr->u.dlci              = md.dlci; /* Keep this as it's needs to get sink during commit phase */
...
static void gaiaTransport_RfcommHandoverCommit(gaia_transport *t, bool is_primary)
{
    ...
    if (is_primary)
    {
        ...
        tr->u.sink = StreamRfcommSinkFromDlci(&t->tp_bd_addr, tr->u.dlci);

        /* TODO: Most of the stuff below should go into transport_mgr */

        /* Set the task for connection */
        uint16 conn_id = SinkGetRfcommConnId(tr->u.sink);
        ...
        DEBUG_LOG_DEBUG("gaiaTransport_RfcommHandoverCommit, primary, sink %u, channel %u, conn_id %u",
                        tr->u.sink, tr->channel, conn_id);

        /* Stitch the RFCOMM sink and the Transport Manager task.
        * This just does 'MessageStreamTaskFromSink(sink, task)'. */
        TransportMgrConfigureSink(tr->u.sink);


        /* Configure Sink */
        SinkConfigure(tr->u.sink, VM_SINK_MESSAGES, VM_MESSAGES_ALL);
        ...
        if (TransportMgrCreateTransLinkData(&tr->common.task, link_cfg, tr->u.sink))
        {
        ...
        Source src = StreamSourceFromSink(tr->u.sink);
...
static void gaiaTransport_RfcommHandleTransportMgrLinkDisconnectedCfm(gaia_transport_rfcomm_t *tr, const TRANSPORT_MGR_LINK_DISCONNECTED_CFM_T *cfm)
{
    ...
    if (cfm->status)
    {
        ...
        else
        {
            ...
            /* Send disconnect confirmation to framework if in disconnecting state, else send indication */
            if (tr->common.state == GAIA_TRANSPORT_DISCONNECTING)
                Gaia_TransportDisconnectCfm(&tr->common, TRUE);
            else
                Gaia_TransportDisconnectInd(&tr->common);
