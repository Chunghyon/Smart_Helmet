...
#ifdef ENABLE_GAIA_SIZE_CHECK
    uint32 max_payload_size;
    if (Gaia_TransportGetInfo(t, GAIA_TRANSPORT_PAYLOAD_SIZE, &max_payload_size) && (payload_length > max_payload_size))
        DEBUG_LOG_WARN("Gaia_TransportSendPacket, payload size %u exceed maximum %u", payload_length, max_payload_size);
#endif
...
void Gaia_TransportConnectInd(gaia_transport *t, bool success, const tp_bdaddr *tp_bd_addr)
{
    ...
    else if (success && t->state == GAIA_TRANSPORT_DISCONNECTING)
...
void Gaia_TransportDisconnectReq(gaia_transport *t)
{
    ...
    else if (t->functions->disconnect_req && t->state == GAIA_TRANSPORT_CONNECTING)
    {
        /* Move to disconnecting state, do actual disconnect once connecting
         * has completed */
        t->state = GAIA_TRANSPORT_DISCONNECTING;
    }
    else
        gaia_TransportSendGaiaDisconnectCfm(t, FALSE);
}



/*! @brief Called from transport to confirm disconnection.
 */
void Gaia_TransportDisconnectCfm(gaia_transport *t, bool success)
{
    PanicNull(t);
...
void Gaia_TransportDisconnectInd(gaia_transport *t)
{
    ...
    if (t->state == GAIA_TRANSPORT_DISCONNECTING)
        gaia_TransportSendGaiaDisconnectCfm(t, TRUE);
    else
